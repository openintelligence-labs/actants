...
def build_server(
    ...
    *,
    ...
    try:
        ...
        raise ImportError("MCP support requires `pip install agentic-kit[mcp]`") from exc

    registry = _registry_from(source)
    server = FastMCP(name=name or "agentic-kit", instructions=instructions)
...
def _registry_from(source: Agent | ToolRegistry) -> ToolRegistry:
    ...
    from agentic_kit.agents.agent import Agent as AgentClass
