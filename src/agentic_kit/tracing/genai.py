...
Cost is namespaced under ``agentic_kit.cost.*`` because the spec does not define
...
def record_response(
    ...
    *,
    ...
    Cost is non-spec — namespaced under ``agentic_kit.cost.usd`` so a future
    ...
    _set(span, "agentic_kit.cost.usd", cost_usd)
    ...
    *,
    ...
    with tracer.start_as_current_span(name) as span:
        ...
        if input_count is not None:
            _set(span, "agentic_kit.embeddings.input_count", input_count)
        yield span
